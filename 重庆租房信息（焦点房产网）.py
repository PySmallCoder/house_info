...
if __name__ =="__main__":
    ...
    houseInfoDict = []
    f =open("d://重庆租房信息(焦点房产网).json","w",encoding = "utf-8")
    header = {
    ...
    start_urls = ["https://esf.focus.cn/cq/rents/?page=%d&size=60" % d for d in range(1,100)]
    for url in start_urls:
        ...
        for houseId in houseIdList:
            ...
            try:
                ...
                #房屋朝向
                communityInfo['aspect'] = response.xpath('//div[@class="sechand-detail-info"]/ul[4]/li[1]/span[2]/text()')[0]
                #楼层
                ...
                #发布时间
                communityInfo['publish_time'] = ''.join(response.xpath('//div[@class="sechand-detail-info"]/ul[2]/li[3]/span[2]/text()')[0])
                #单价
                #communityInfo['unit_price'] = response.xpath('//div[@class="sechand-info"]/div/div/span//text()')
                #月租
                communityInfo['monthly_rent'] = ''.join(response.xpath('//div[@class="sechand-info"]/div/h4//text()'))
                #房屋标签
