...
if __name__ =="__main__":
    ...
    start_urls = ["https://cd.sp.anjuke.com/zu/p%d/" % d for d in range(1,91)]
    for url in start_urls:
        ...
        for url in urls:
            ...
            try:
                ...
                #地址
                communityInfo['address'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li/span[@class="desc addresscommu"]/@title')[0].split())
                #面积
                communityInfo['area'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li[4]/span[2]/text()'))
                #楼层
                communityInfo['floor'] = ''.join(response.xpath('//div[@id="fy_info"]//ul/li[8]/span[2]//text()'))
                #物业费
                ...
                #人群
                communityInfo['personas'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li/span[@class="desc"]/@title')[1]).strip()
                #是否临街
...
with open("d://房产//成都商铺出租信息.json","w",encoding = "utf-8") as f:
