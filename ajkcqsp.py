...
if __name__ =="__main__":
    ...
    start_urls = ["https://cq.sp.anjuke.com/zu/p%d/" % d for d in range(1,31)]
    for url in start_urls:
        ...
        for url in urls:
            ...
            try:
                ...
                #月租金
                communityInfo['month_rent'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li[1]/span[2]/text()'))
                #楼盘名
                communityInfo['house_name'] = ''.join(response.xpath("//h1[@class='tit-name']/text()"))
                #地址
                communityInfo['address'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li[13]/span[2]/text()')[0].split())
                #面积
                ...
                #转让费
                communityInfo['transfer_money'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li[2]/span[2]/text()'))
                #面宽
                communityInfo['house_wide'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li[5]/span[2]/text()'))
                #人群
                communityInfo['personas'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li[11]/span[2]/text()')).strip()
                #楼层
                communityInfo['floor'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li[8]/span[2]/text()'))
                #是否临街
                communityInfo['frontage'] = '临街'
                #层高
                communityInfo['high'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li[6]/span[2]/text()'))
                #状态
                communityInfo['status'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li[9]/span[2]/text()'))
                #押付
                communityInfo['payment_type'] = re.findall(r'付[0-9]{1}押[0-9]{1}',html)[0]
                #房源描述
                communityInfo['describe'] = ''.join(response.xpath('//div[@class="desc-con"]//p//text()'))
                #链接
...
with open("d://房产//商铺出租信息.json","w",encoding = "utf-8") as f:
