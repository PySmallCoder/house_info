...
def parse_link(html):
    ...
    urls = response.xpath('//h3[@class="house-title"]/a/@href')
    return urls

if __name__ =="__main__":

    f = open("d://重庆租房信息(乐居网).json","w",encoding = "utf-8")
    ...
    cookie = {
        "currentpos":"106.60374|29.52942",
        "esf_bi_touch_first":"1",
        "esf_bi_touch_lejupsid":"23a1e253ad840fabe651ad6885f905d2",
        "esf_bi_touch_lejustep":"1",
        "esf_bi_touch_m_source_id":"0",
        "esf_bi_touch_psid	":"d2f2d6430d8eb633eb5852eedac6f56f",
        "esf_bi_touch_refresh_num	":"0",
        "esf_bi_touch_start_time":"1527511941066",
        "esf_bi_touch_step":"1",
        "fnjAdIsFirst": "1",
        "LEJUGLOBAL":"23a1e253ad840fabe651ad6885f905d2",
        "PHPSESSID":"d2f2d6430d8eb633eb5852eedac6f56f"
    }
    start_urls = ["https://cq.zufang.leju.com/house/n%d/" % d for d in range(1,4)]
    for url in start_urls:
        ...
        for houseId in houseIdList:
            ...
            try:
                ...
                #标题
                communityInfo['title'] = ''.join(response.xpath('//div[@class="centered"]/h1/text()')[0].split())
                #所属小区
                communityInfo['community'] = response.xpath('//div[@class="aroundInfo"]/div[3]/div/a[1]//text()')[0].strip()
                #所属位置
                communityInfo['position'] = ''.join(''.join(response.xpath('//div[@class="aroundInfo"]/div/div/p/text()')).split())
                #建造年代
                communityInfo['year'] = ''.join(''.join(response.xpath('//div[@class="aroundInfo"]/div[3]/div/text()')).split())
                #建筑类型
                communityInfo['house_type'] = response.xpath('//div[@class="room fl"]/div[2]/text()')[0].strip()
                #房屋户型
                communityInfo['house_style'] = ''.join(response.xpath('//div[@class="room fl"]/div[1]/text()')).strip()
                #面积
                communityInfo['area'] = response.xpath('//div[@class="size fl"]/div[1]/text()')[0]
                #室内面积
                #communityInfo['indoor_area'] = response.xpath('//div[@class="base"]/div[2]/ul/li[5]/text()')[0].strip()
                #房屋朝向
                communityInfo['aspect'] = response.xpath('//div[@class="room fl"]/div[2]/text()')[0].strip()
                #楼层
                communityInfo['floor'] = response.xpath('//div[@class="size fl"]/div[2]/text()')[0].strip()
                #总价
                #communityInfo['all_price'] = ''.join(response.xpath('//div[@class="sechand-info"]/div/h4//text()'))
                #发布时间
                communityInfo['publish_time'] = response.xpath('//div[@class="centered"]/div[2]/span[1]/font/text()')[0]
                #单价
                #communityInfo['unit_price'] = response.xpath('//div[@class="sechand-info"]/div/div/span//text()')
                #月租
                communityInfo['monthly_rent'] = response.xpath('//div[@class="rantal-price"]//text()')[1].strip()
                #房屋标签
                ...
                #付款方式
                communityInfo['payment_style'] = ''.join(''.join(response.xpath('//div[@class="aroundInfo"]/div[2]/text()')).split())
                #装修程度
                communityInfo['decoration_level'] = response.xpath('//div[@class="type fl"]/div[2]/text()')[0].strip()
                #描述
