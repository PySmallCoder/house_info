...
if __name__ =="__main__":
    ...
    start_urls = ["http://zu.cq.fang.com/house/i3%d/" % d for d in range(1,101)]
    for url in start_urls:
        ...
        for houseId in houseIdList:
            ...
            url = "http://zu.cq.fang.com" + houseId
            ...
            try:
                ...
                #标题
                communityInfo['title'] = ''.join(response.xpath('//div[@class="tab-cont clearfix"]/div[@class="title"]/text()')[0].split())
                #所属小区
                communityInfo['community'] = response.xpath('//div[@class="rcont"]//a/text()')[:-1]
                #所属位置
                communityInfo['position'] = response.xpath('//div[@class="rcont"]//a/text()')[-1]
                #建造年代
                #communityInfo['year'] = ''.join(response.xpath('//div[@class="cont clearfix"]/div[1]/span[2]/text()')).strip()
                #建筑类型
                #communityInfo['house_type'] = response.xpath('//div[@class="cont clearfix"]/div[6]/span[2]/text()')[0].strip()
                #房屋户型
                communityInfo['house_style'] = ''.join(response.xpath('//div[@class="tab-cont-right"]/div[3]/div[2]/div[1]/text()')[0].split())
                #面积
                communityInfo['area'] = response.xpath('//div[@class="tab-cont-right"]/div[3]/div[3]/div[1]/text()')[0].strip()
                #室内面积
                #communityInfo['indoor_area'] = response.xpath('//div[@class="base"]/div[2]/ul/li[5]/text()')[0].strip()
                #房屋朝向
                communityInfo['aspect'] = response.xpath('//div[@class="tab-cont-right"]/div[4]/div[1]/div[1]/text()')[0].strip()
                #楼层
                communityInfo['floor'] = response.xpath('//div[@class="tab-cont-right"]/div[4]/div[2]/div[1]/text()')[0].strip()
                #总价
                #communityInfo['all_price'] = ''.join(response.xpath('//div[@class="trl-item_top"]/div//text()')[:2])
                #发布时间
                publishTime = ''.join(''.join(response.xpath('//p[@class="gray9 fybh-zf"]/span[2]/text()')).split())
                ...
                #月租
                communityInfo['monthly_rent'] = ''.json(response.xpath('//div[@class="tab-cont-right"]/div[2]/div[1]//text()'))
                #首付
                #communityInfo['first_payment'] = ""
                #付款方式
                #communityInfo['payment_style'] = ''.join(''.join(response.xpath('//div[@class="trl-item_top"]/div[2]//text()')).split())
                #装修程度
                communityInfo['decoration_level'] = response.xpath('//div[@class="tab-cont-right"]/div[4]/div[3]/div[1]/text()')[0].strip()
                #描述
                communityInfo['descrip'] = ''.join(''.join(response.xpath('//ul[@class="fyms_modify"]/li/div//text()')).split())
                #出租方式
                #communityInfo['rent_style'] = ''
                #地铁
