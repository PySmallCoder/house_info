...
if __name__ =="__main__":
    ...
    cookie = {
        ...
        'twe': '2;',
        'sessid': '2CB9723D-A26C-2A4B-703A-9CF231942AE1;',
        '__xsptplusUT_8': '1;',
        'new_uv': '12;',
        'new_session':'0',
    ...
    start_urls = ["https://cd.xzl.anjuke.com/zu/p%d/" % d for d in range(1,90)]
    for url in start_urls:
        ...
        for url in urls:
            ...
            try:
                ...
                #日租金
                communityInfo['data_rent'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li[1]/span[2]/text()'))
                #月租金
                communityInfo['month_rent'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li[2]/span[2]/text()'))
                #楼盘名
                communityInfo['house_name'] = response.xpath('//div[@id="fy_info"]/ul[1]/li[4]/span[2]/a/text()')
                #地址
                communityInfo['address'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li[5]/span[2]/text()'))
                #地铁
                communityInfo['subway'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[1]/li[6]/span[2]/text()'))
                #面积
                communityInfo['area'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[2]/li[1]/span[2]/text()'))
                #楼层
                communityInfo['floor'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[2]/li[2]/span[2]/text()'))
                #工位数
                communityInfo['work_number'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[2]/li[3]/span[2]/text()'))
                #物业费
                communityInfo['property_fee'] = ''.join(response.xpath('//div[@id="fy_info"]/ul[2]/li[4]/span[2]/text()')).strip()
                #空调类型
                communityInfo['air_conditioner'] = ''.join(response.xpath('//div[@class="item-mod"]/div/ul[1]/li[5]/span[2]/text()'))
                #车位
                communityInfo['truck_space'] = ''.join(response.xpath('//div[@class="item-mod"]/div/ul[1]/li[6]/span[2]/text()'))
                #物业公司
                communityInfo['property_company'] = ''.join(response.xpath('//div[@class="item-mod"]/div/ul[2]/li[3]/span[2]/text()'))
                #电梯,
                communityInfo['elevator'] = ''.join(response.xpath('//div[@class="item-mod"]/div/ul[2]/li[5]/span[2]/text()'))
                houseInfoDict.append(communityInfo)
                print(communityInfo)
            except:
                print("出错啦",url)

with open("d://房产//成都写字楼出租信息.json","w",encoding = "utf-8") as f:
