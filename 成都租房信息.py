...
def parse_link(html):
    ...
    urls = response.xpath('//div[@class="zu-info"]/h3/a/@href')
    return urls
...
if __name__ =="__main__":
    ...
    start_urls = ["https://cd.zu.anjuke.com/fangyuan/p%d/" % d for d in range(1,51)]
    for url in start_urls:
        ...
        for url in urls:
            ...
            try:
                ...
                #标题
                communityInfo['title'] = ''.join(response.xpath("//h3[@class='house-title']/text()"))
                #租金
                communityInfo['rent'] = ''.join(response.xpath('//div[@class="lbox"]/ul/li[1]/span//text()'))
                #地址
                communityInfo['address'] = ''.join(response.xpath('//div[@class="lbox"]/ul/li[8]/a//text()')[0].split())
                #面积
                communityInfo['area'] = ''.join(response.xpath('//div[@class="lbox"]/ul/li[3]/span[2]/text()'))
                #楼层
                communityInfo['floor'] = ''.join(response.xpath('//div[@class="lbox"]/ul/li[5]/span[2]/text()'))
                # 房屋户型
                communityInfo['house_style'] = ''.join(response.xpath('//div[@class="lbox"]/ul/li[2]/span[2]/text()')[0].split())
                # 装修程度
                communityInfo['decoration_level'] = response.xpath('//div[@class="lbox"]/ul/li[6]/span[2]/text()')[0].strip()
                # 房屋朝向
                communityInfo['aspect'] = response.xpath('//div[@class="lbox"]/ul/li[4]/span[2]/text()')[0].strip()
                # 房屋类型
                communityInfo['house_type'] = response.xpath('//div[@class="lbox"]/ul/li[7]/span[2]/text()')[0].strip()
                #房源描述
                communityInfo['describe'] = ''.join(response.xpath('//div[@class="auto-general"]//text()')).strip().strip()
                #要求
                communityInfo['require'] = ''.join(response.xpath('//div[@class="lbox"]/ul/li[9]/span[2]/text()'))
                #链接
...
with open("d://房产//成都租房信息.json","w",encoding = "utf-8") as f:
