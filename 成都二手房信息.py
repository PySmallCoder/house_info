...
def parse_link(html):
    ...
    urls = response.xpath('//ul[@id="houselist-mod-new"]//li/div[2]/div/a/@href')
    return urls
...
if __name__ =="__main__":
    ...
    start_urls = ["https://chengdu.anjuke.com/sale/p%d/" % d for d in range(1,27)]
    for url in start_urls:
        ...
        for url in urls:
            ...
            try:
                ...
                #标题
                communityInfo['title'] = ''.join(response.xpath('//h3[@class="long-title"]/text()')[0].split())
                #所属小区
                communityInfo['community'] = response.xpath('//div[@class="houseInfo-wrap"]/div/div/dl/dd/a/text()')[0].strip()
                #所属位置
                communityInfo['position'] = ''.join(''.join(response.xpath('//div[@class="houseInfo-wrap"]/div/div/dl[2]/dd/p//text()')).split())
                #建造年代
                ...
                #描述
                communityInfo['descrip'] = ''.join(''.join(response.xpath('//div[@class="houseInfo-desc"]/div[3]//text()')).split())
                #链接
...
with open("d://房产//成都二手房信息.json","w",encoding = "utf-8") as f:
